...
public class InitDatabase {
    ...
    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        ...
        try {
            Connection conn = DriverManager.getConnection(
                    "jdbc:mysql://localhost:3306/hy359_2024", "root", "");
    ...
    public void initDatabase() throws SQLException, ClassNotFoundException {
        ...
        String checkDatabaseQuery = "SHOW DATABASES LIKE 'HY359_2024'";
        ResultSet rs = stmt.executeQuery(checkDatabaseQuery);

        if (rs.next()) {
            // Αν η βάση δεδομένων υπάρχει
            System.out.println("Database HY359_2024 already exists. Skipping creation.");
        } else {
            // Αν η βάση δεδομένων δεν υπάρχει, τη δημιουργούμε
            String createDatabaseQuery = "CREATE DATABASE HY359_2024";
            stmt.executeUpdate(createDatabaseQuery);
            System.out.println("Database HY359_2024 created successfully.");
        }
        rs.close();
        stmt.close();
        conn.close();
